...
source 関数でデータを抽出しないでください。可能であれば、そのタスクはリソースに任せてください。ソース関数は、呼び出されるとすぐに実行されます (Python ジェネレーターなどの実行を遅らせるリソースとは異なります)。`pipeline.run` または `pipeline.extract` 内でデータを抽出すると、いくつかの利点 (エラー処理、実行メトリック、並列化) が得られます。
...
ソースを作成した後で、カスタムリソースをソースに追加できます。すべての取引を Keras モデルでスコアリングして、取引が詐欺であるかどうかを判断したいとします。そのためには、新しい `deals` リソースから[データを取得するトランスフォーマー](resource.md#process-resources-with-dlttransformer) を宣言し、ソースに追加します。
